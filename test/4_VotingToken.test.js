...
contract("BEP20Votes", function (accounts) {
  ...
  const name = "Soroosh Smart Ecosystem";
  const symbol = "SSE";
  ...
  describe("set delegation", function () {
    ...
    it("voting power calculation", async function () {
      ...
      expect(await this.token.numCheckpoints(recipient)).to.be.bignumber.equal(
        "1"
      );
      ...
      expect(await this.token.numCheckpoints(other1)).to.be.bignumber.equal(
        "1"
      );
      ...
      expect(await this.token.delegates(recipient)).to.be.bignumber.equal(
        other1
      );
  ...
  describe("delegateBySig", function () {
    ...
    const Domain = (SSE) => ({ name, chainId, verifyingContract: SSE.address });
    ...
    it("accept signed delegation", async function () {
      ...
      expect(await this.token.delegates(delegatorAddress)).to.be.equal(
        ZERO_ADDRESS
      );
      ...
      expect(await this.token.getVotes(delegatorAddress)).to.be.bignumber.equal(
        supply
      );
      expect(
    ...
    it("rejects reused signature", async function () {
      ...
      await expectRevert(
        ...
        "SSE::delegateBySig: invalid nonce"
      );
    });
  ...
  describe("change delegation", function () {
    ...
    it("call", async function () {
      ...
      expect(await this.token.getVotes(holderDelegatee)).to.be.bignumber.equal(
        supply
      );
      expect(
        await this.token.getPastVotes(holder, receipt.blockNumber - 1)
  ...
  describe("transfers", function () {
    ...
    it("no delegation", async function () {
      ...
      const { receipt } = await this.token.transfer(other2, 1, {
        from: other1,
      });
      expectEvent(receipt, "Transfer", {
        from: other1,
  ...
  describe("numCheckpoints", function () {
    ...
    it("returns the number of checkpoints for a delegate", async function () {
      ...
      await time.advanceBlock();
      expect(
        await this.token.getPastVotes(other1, t1.receipt.blockNumber)
      ).to.be.bignumber.equal("100");
      expect(
        await this.token.getPastVotes(other1, t2.receipt.blockNumber)
      ).to.be.bignumber.equal("90");
      expect(
        await this.token.getPastVotes(other1, t3.receipt.blockNumber)
      ).to.be.bignumber.equal("80");

      expect(
        await this.token.getPastVotes(other1, t4.receipt.blockNumber)
      ).to.be.bignumber.equal("80");
    });
  });
  ...
  describe("getPastVotes", function () {
    ...
    it("returns 0 if there are no checkpoints", async function () {
      expect(await this.token.getPastVotes(other1, 0)).to.be.bignumber.equal(
        "0"
      );
    });
